...
static void _do_call(em_queued_call *q)
{
	if (!q->js) {
		...
		// JS call
		q->returnValue.d = emscripten_receive_on_main_thread_js((int)q->functionPtr, EM_FUNC_SIG_NUM_FUNC_ARGUMENTS(q->functionEnum), &q->args[0].d);
	}
...
static void prepare_js_proxying(em_queued_call* q, int index, int num_args, double* buffer)
{
	q->functionEnum = EM_FUNC_SIG_WITH_N_PARAMETERS(num_args);
	q->functionPtr = (void*)index;
	q->js = 1;
	// We write out the JS doubles into args[], which must be of appropriate size -
	// JS will assume that.
