...
def get(ports, settings, shared):
  ...
  def create():
    ...
    source_include_path = os.path.join(source_path, 'include', 'curl')
    ports.install_headers(source_include_path, target='curl')

    shared.safe_ensure_dirs(os.path.join(dest_path, 'lib'))
    open(os.path.join(dest_path, 'lib', 'curl_config.h'), 'w').write(curl_config_h)
    ...
    for src_path in srcs:
      ...
      o_file = os.path.join(ports.get_build_dir(), 'curl', src_base + '.o')
    ...
    final = os.path.join(ports.get_build_dir(), 'curl', libname)
